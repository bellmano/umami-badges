...
describe("API Functions", () => {
  ...
  test("resolveUmamiUrl handles empty, invalid, and valid inputs", () => {
    ...
    expect(resolveUmamiUrl("http://umami.example.com/api/v1")).toBe(
      "http://umami.example.com/api",
    );
    expect(resolveUmamiUrl("https://umami.example.com/api/v1/")).toBe(
      "https://umami.example.com/api",
    );
    expect(resolveUmamiUrl("https://api.umami.is/v1")).toBe(
  ...
  test("formatMetricValue formats all metric types and handles edge cases", () => {
    // All metric types
    expect(formatMetricValue({ pageviews: 1500 }, "views").formattedValue).toBe(
      "1.5K",
    );
    expect(
      formatMetricValue({ visitors: 2500000 }, "visitors").formattedValue,
    ).toBe("2.5M");
    expect(formatMetricValue({ visits: 800 }, "visits").formattedValue).toBe(
      "800",
    );
    expect(
      formatMetricValue({ bounces: 50, visits: 100 }, "bounce-rate")
        .formattedValue,
    ).toBe("50.0%");
    expect(
      formatMetricValue({ totaltime: 200, visits: 100 }, "avg-session")
        .formattedValue,
    ).toBe("2s");
    expect(
      formatMetricValue({ totals: { visitors: 42 } }, "live").formattedValue,
    ).toBe("42");
    expect(
      formatMetricValue({ totals: { visitors: 1500 } }, "realtime")
        .formattedValue,
    ).toBe("1.5K");
    ...
    expect(
      formatMetricValue({ bounces: 10, visits: 0 }, "bounce-rate")
        .formattedValue,
    ).toBe("1000.0%");
    expect(
      formatMetricValue({ bounces: 5 }, "bounce-rate").formattedValue,
    ).toBe("500.0%");
    expect(
      formatMetricValue({ bounces: 3, visits: null }, "bounce-rate")
        .formattedValue,
    ).toBe("300.0%");
    expect(
      formatMetricValue({ bounces: 8, visits: -5 }, "bounce-rate")
        .formattedValue,
    ).toBe("800.0%");
    expect(
      formatMetricValue({ totaltime: 5, visits: 0 }, "avg-session")
        .formattedValue,
    ).toBe("5s");
    expect(
      formatMetricValue({ totaltime: 96, visits: 1 }, "avg-session")
        .formattedValue,
    ).toBe("1m 36s");
  ...
  describe("fetchUmamiData", () => {
    ...
    test("handles realtime endpoint errors", async () => {
      ...
      await expect(
        fetchUmamiData("https://api.umami.is/v1", "test", "token", "live"),
  ...
  describe("Express Routes", () => {
    ...
    test("/api/:metric validates parameters and generates badges", async () => {
      const handler = app.router.stack.find(
        (r) => r.route?.path === "/api/:metric",
      ).route.stack[0].handle;
      ...
      expect(mockRes.redirect).toHaveBeenCalledWith(
        expect.stringContaining("Missing%20Website%20ID"),
      );
      ...
      await handler(mockReq, mockRes);
      expect(mockRes.redirect).toHaveBeenCalledWith(
        expect.stringContaining("Missing%20API%20Token"),
      );
      ...
      await handler(mockReq, mockRes);
      expect(mockRes.redirect).toHaveBeenCalledWith(
        expect.stringContaining("Invalid%20Umami%20URL"),
      );
    ...
    test("/api/:metric handles errors with style fallback", async () => {
      ...
      await handler(mockReq, mockRes);
      expect(mockRes.redirect).toHaveBeenCalledWith(
        expect.stringContaining("Failed%20to%20fetch"),
      );
      expect(mockRes.redirect).toHaveBeenCalledWith(
        expect.stringContaining("style=flat"),
      );
    ...
    test("/api/:metric handles complete error fallback with redirect", async () => {
      ...
      expect(mockRes.redirect).toHaveBeenCalledWith(
        expect.stringContaining("style=plastic"),
      );
    });
  });
});
