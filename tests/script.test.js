...
describe("Badge Generator", () => {
  ...
  test("validates required fields and generates badge with all parameter combinations", () => {
    ...
    expect(global.alert).toHaveBeenCalledWith(
      "Please fill in the required fields: (Website ID and API Key)",
    );
    ...
    document.getElementById("umamiUrl").value =
      "https://umami.example.com/api/v1";
    form.dispatchEvent(new Event("submit"));
    ...
    expect(url).toContain(
      "umamiUrl=https%3A%2F%2Fumami.example.com%2Fapi%2Fv1",
    );

    // Test result display
    expect(
      document.getElementById("resultSection").classList.contains("hidden"),
    ).toBe(false);
    expect(Element.prototype.scrollIntoView).toHaveBeenCalled();
    expect(document.getElementById("badgePreview").src).toContain("/api/views");
    expect(document.getElementById("markdownCode").value).toContain(
      "![Custom]",
    );
  ...
  test("uses default labels for all metrics and handles unknown metrics", () => {
    ...
    metrics.forEach(({ value, label }) => {
      ...
      expect(document.getElementById("htmlCode").value).toContain(
        `alt="${label}"`,
      );
    });
  ...
  test("loads saved theme on init and handles missing theme and missing element", () => {
    ...
    expect(
      document.getElementById("theme-toggle").querySelector("i"),
    ).toBeTruthy();
